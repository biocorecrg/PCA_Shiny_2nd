...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      fileInput("file_pca", "Upload expression matrix", accept = c("text/txt", "text/csv")),
      # input file: sample table that will tell what groups samples belong to
      fluidRow(column(9, fileInput("sample_table", "Upload sample table file", accept = c("text/txt", "text/csv"))),
      ...
      bsPopover(id = "q1", title = "sample table",
                content = paste0("file that contains columns with samples grouping and other characteristics. The first column must correspond to the column names of the input file!"),
                placement = "right", 
                trigger = "click", 
      ...
      # Plot title
      textInput("pca_title", label = h5("Plot title"), value = "Principal Component Analysis"),
      ...
      numericInput("label_size", label = h5("Point label size:"), value = 2, min=0.5, max=20, step=0.5)
    )
    ,
...
server <- function(input, output, session) {
  ...
  observe({
    if(!is.null(input$file_pca)){
      ...
      output$pca_columns1 <- renderUI({
        numericInput("pca_col1", 
                     label = h5("Principal component on x-axis:"), 
                     value = 1,
                     min=1, 
                     max=10, 
                     step=1)
      ...
      output$pca_columns2 <- renderUI({
        numericInput("pca_col2", 
                     label = h5("Principal component on y-axis:"), 
                     value = 2,
                     min=1,
                     max=10,
                     step=1)
        ...
        tabsetPanel(
          ...
          tabPanel("Selected samples", 
                   checkboxGroupInput("sampleschecks", 
                                      label=h5("Samples selected: unselect and refresh to update the plot"), 
    ...
    if(!is.null(input$sample_table)){
      if(unique(input_sampletable()[,1] %in% as.character(colnames(input_pca()))) == TRUE){
        output$sample_grouping_factor <- renderUI({
          selectInput("group_color", label = h5("Select column by which points will be colored"),
                      choices =  colnames(input_sampletable())[-1], 
                      selected = 1)
          })
        output$sample_point_labels <- renderUI({
          selectInput("point_labels", label = h5("Select column by which points will be named"),
                      choices =  colnames(input_sampletable()), 
                      selected = 1)
        })
      }else{
  ...
  drawPCA <- reactive({
    ...
    # if there is a sample table file, choose grouping by color and labels
    if (!is.null(input$file_pca)){
    ...
    if(!is.null(input$file_pca)){
      ...
      colnames(df_pca)[grep(paste0("^",input$group_color,"$"), colnames(df_pca))] <- "groups"
      colnames(df_pca)[grep(paste0("^",input$sample_point_labels,"$"), colnames(df_pca))] <- "names"
      p_pca <- ggplot(data=df_pca, aes(x=PC1, y=PC2, label=names, color=groups))
    }else{
      df_pcs <- df_pca0
    ...
    p_pca + geom_point(size=3) +
      ...
      geom_text(size=input$label_size, hjust=-0.2) +
      xlim(xlimits) +
